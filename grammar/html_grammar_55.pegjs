...
EventAttribute "event attribute" = ( '@'name:HTMLAttrName event_modifiers:('.'EventModifiers)* text:(__ '=' __ s:VariableQuoteString) __ { return { name: name, value: (text && text[3]) ? text[3] : "", type:"event_attr", category: "html_attribute", modifiers:event_modifiers.map(v=>v[1])}} )
                                   ...
                                   / ( '@'name:MouseEvent mouse_keys:(':' (KeyboardSystemModifiersKeys'.' / ('.'KeyboardSystemModifiersKeys))* MouseClick)? event_modifiers:('.' ( KeyboardSystemModifiersKeys / EventModifiers ))* text:(__ '=' __ s:VariableQuoteString) __ { return { name: name, value: (text && text[3]) ? text[3] : "", type:"event_attr", category: "html_attribute", modifiers:event_modifiers.map(v=>v[1]), mouse_keys:mouse_keys ? mouse_keys.filter((v,idx)=>{return idx > 0}) : []}} ) 
...
CurrentHtmlAttrName = 'accept-charset'i / 'accesskey'i / 'action'i / 'align'i / 'allow'i / 'alt'i / 'async'i
                       ...
                       / 'formenctype'i / 'formmethod'i / 'formnovalidate'i / 'fn''formtarget'i / 'headers'i / 'hidden'i
