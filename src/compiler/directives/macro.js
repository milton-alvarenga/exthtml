...
function idname(attr,mode,result,variableName,node, parent_nm) {
    ...
    if( mode == "STATIC") {
        result.code.create.push(`$$_setAttr(${variableName}, 'id', '${attr.value}')`)
        result.code.create.push(`$$_setAttr(${variableName}, 'name', '${attr.value}')`)
    } else {
        ...
        result.code.reactives.push(`function ${reactiveFnName}(){\n
            $$_setAttr(${variableName}, 'id', ${attr.value});\n
            $$_setAttr(${variableName}, 'name', ${attr.value});\n
        }`)
