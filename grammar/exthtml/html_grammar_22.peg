...
<input type="submit" attr="ok">
<input type="submit" @event="fn">
<input type="submit" attr="ok" attr2="ok2">
<input type="submit" attr={ok} @event="fn">
-->
 ...
 */
Element  = RawText / Nested / SelfClose / Comment / DocType / Component / DynamicTextNode / TextNode
 ...
 */
SelfClose "self close element" = '<' name:SelfCloseTagName __ attrs:Attributes __ CloseTag __ { console.log(attrs); return { type: 'SELF_CLOSE_TAG', value: name.toUpperCase(), attrs:attrs.filter((v) => { return v.type == 'attr'}).map( (v) => { delete v.type; return v}), dynamic_attrs:attrs.filter((v) => { return v.type == 'dyn_attr'}).map( (v) => { delete v.type; return v}), event_attrs:attrs.filter((v) => { return v.type == 'event_attr'}).map( (v) => { delete v.type; return v}), directives:[], children:[], location: location() }; }
...
TextNode "text node" = v:Text !VariableQuoteString { return { type: 'TEXT_NODE', value: v.join(""), attrs:[], event_attrs:[], directives:[], children:[], location: location() }; }
...
Text "text"
  ...
  / ch:(!TagEnd !SelfClose !Comment !DocType !VariableQuoteString c:.{ return c })+

/**
 * Element attributes
 */
Attributes = __ attrs:( Attribute / DynamicAttribute / EventAttribute )* __ { return attrs.flat(); }

Attribute "attribute" = name:AttrName __ text:(__ '=' __ s:DoubleQuoteString)? attrs:( Attributes / (__ &CloseTag __) ) { let a = [{ name: name, value: (text && text[3]) ? text[3] : "", type:"attr"}]; if(attrs.length){ a.push(attrs) } return a.flat(2)}

DynamicAttribute "dynamic attribute" = name:AttrName __ text:(__ '=' __ s:VariableQuoteString)? attrs:( Attributes / (__ &CloseTag __) ) { let a = [{ name: name, value: (text && text[3]) ? text[3] : "", type:"dyn_attr"}]; if(attrs.length){ a.push(attrs) } return a.flat(2)}

EventAttribute "event attribute" = '@'name:AttrName __ text:(__ '=' __ s:String)? attrs:( Attributes / (__ &CloseTag __) ) { let a = [{ name: name, value: (text && text[3]) ? text[3] : "", type:"event_attr"}]; if(attrs.length){ a.push(attrs) } return a.flat(2)}
