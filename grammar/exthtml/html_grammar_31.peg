...
Nested    "element"   = begin:TagBegin __ children:Element* __ end:TagEnd __ { begin.location = location(); begin.children.push(...children); return begin; }
...
Component "component" = '<' name:TagName attrs:Attributes? CloseTag __ { return { type: 'COMPONENT', value: name.toUpperCase(), attrs:attrs.filter((v) => { return v.type == 'attr'}).map( (v) => { delete v.type; return v}), dynamic_attrs:attrs.filter((v) => { return v.type == 'dyn_attr'}).map( (v) => { delete v.type; return v}), event_attrs:attrs.filter((v) => { return v.type == 'event_attr'}).map( (v) => { delete v.type; return v}), directives:[], children:[], location: location() }; }
...
Text "text"
  ...
  / ch:(!TagEnd !SelfClose !Comment !DocType !DynamicTextNode !Component !Nested !RawText c:.{ return c })+

/**
 * Element attributes
 */
Attributes = __ attrs:( DynamicAttribute / EventAttribute / StaticAttribute)* __ &CloseTag __ { return attrs; }
...
Html4ExclusivesTagName = 'acronym'i / 'applet'i / 'big'i / 'center'i / 'dir'i / 'font'i / 'frame'i / 'frameset'i
	/ 'noframes'i / 'strike'i / 'tt'

Html5TagName = 'a'i / 'abbr'i / 'address'i / 'area'i / 'article'i / 'aside'i / 'audio'i / 'b'i / 'base'i
	/ 'basefont'i / 'bdi'i / 'bdo'i / 'blockquote'i / 'body'i / 'br'i / 'button'i / 'canvas'i / 'caption'i
    ...
    / 'hr'i / 'html'i / 'i'i / 'iframe'i / 'img'i / 'input'i / 'ins'i / 'kbd'i / 'label'i / 'legend'i
    / 'li'i / 'link'i / 'main'i / 'map'i / 'mark'i / 'meta'i / 'meter'i / 'nav'i / 'noscript'i
    / 'object'i / 'ol'i / 'optgroup'i / 'option'i / 'output'i / 'p'i / 'param'i / 'picture'i / 'pre'i
    / 'progress'i / 'q'i / 'rp'i / 'rt'i / 'ruby'i / 's'i / 'samp'i / 'script'i / 'section'i / 'select'i
    / 'small'i / 'source'i / 'span'i / 'strong'i / 'style'i / 'sub'i / 'summary'i / 'sup'i / 'svg'i
    / 'table'i / 'tbody'i / 'td'i / 'template'i / 'textarea'i / 'tfoot'i / 'th'i / 'thead'i / 'time'i
    / 'title'i / 'tr'i / 'track'i / 'u'i / 'ul'i / 'var'i / 'video'i / 'wbr'i
