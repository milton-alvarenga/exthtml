...
Component "component" =  ( '<' name:TagName attrs:Attributes? SelfCloseTag __ { return { type: 'COMPONENT', value: name.toUpperCase(), attrs:attrs.filter((v) => { return v.type == 'attr'}).map( (v) => { delete v.type; return v}), dynamic_attrs:attrs.filter((v) => { return v.type == 'dyn_attr'}).map( (v) => { delete v.type; return v}), event_attrs:attrs.filter((v) => { return v.type == 'event_attr'}).map( (v) => { delete v.type; return v}), directives:[], children:[], location: location() }; } )
/*
 ...
 * Element attributes
 */
Attributes = __ attrs:( DynamicAttribute / EventAttribute / StaticAttribute)* __ &GeneralCloseTag __ { return attrs; }
