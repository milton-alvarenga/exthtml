...
NestedComponent = begin:TagBegin __ children:Element* __ end:TagEnd __ { return { type: 'COMPONENT', value: begin.value, attrs:begin.attrs.filter((v) => { return v.type == 'attr'}).map( (v) => { delete v.type; return v}), dynamic_attrs:begin.attrs.filter((v) => { return v.type == 'dyn_attr'}).map( (v) => { delete v.type; return v}), event_attrs:begin.attrs.filter((v) => { return v.type == 'event_attr'}).map( (v) => { delete v.type; return v}), directives:[], children:[...children], location: location() }; }
